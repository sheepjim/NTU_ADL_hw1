...
def main():
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        logger.info(result)
        rouge1.append(result['rouge-1']['f'])
        rouge2.append(result['rouge-2']['f'])
        rougeL.append(result['rouge-l']['f'])
    ...
    if args.output_dir is not None:
        ...
        if accelerator.is_main_process:
            ...
            with open(os.path.join(args.output_dir, "report.txt"), 'a') as f:
                f.write(" ".join(str(val) for val in rouge1))
                f.write("\n")
                f.write(" ".join(str(val) for val in rouge2))
                f.write("\n")
                f.write(" ".join(str(val) for val in rougeL))
